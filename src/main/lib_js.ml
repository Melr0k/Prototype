...
let json_of_pos pos =
  ...
  `Assoc [ ("startLine"  , `Int (line startp)  )
         ; ("startCol"   , `Int (column startp))
         ; ("endLine"    , `Int (line endp)    )
         ; ("endCol"     , `Int (column endp)  )
         ; ("startOffset", `Int (offset startp))
         ; ("endOffset"  , `Int (offset endp)  ) ]
      ...
      | PSuccess (tenv, lst) ->
        ...
        let (_, res) =
          List.fold_left (fun (env, res) (_, (v, e, ta)) ->
            ...
            let def_pos = Parsing.Variable.Variable.get_locations v
                          |> List.hd in
            let (env, res) =
              ...
              | TFailure (pos, msg, (tmsc, ttype)) ->
                ...
                let untyp =
                  `Assoc [ ("name"    , `String name        )
                         ...
                         ; ("typeable", `Bool false         )
                         ; ("message" , `String msg         )
                         ; ("pos"     , json_of_pos_list pos) ]
                in
                (env, untyp::res)
              in
              if Js.Opt.test callback then (
                let intermediate_answer =
                  ok_answer res |> to_string |> Js.string in
                Js.Unsafe.fun_call callback
                  [| intermediate_answer |> Js.Unsafe.inject |]
                |> ignore
              ) ;
              (env, res)
            ) (initial_env, []) lst
        ...
        ok_answer res
      | PFailure (pos, msg) ->
         `Assoc [ ("exit_code", `Int (-2)             )
                ; ("message"  , `String msg           )
                ; ("pos"      , json_of_pos_list [pos]) ]
    ) with e ->
      `Assoc
        [ ("exit_code", `Int (-1)                                          )
        ; ("message"  , `String ("internal error: "^(Printexc.to_string e)))
        ; ("pos"      , `List []                                           )
        ]
  in
