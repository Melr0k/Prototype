...
let instantiate_alias env args name =
  ...
  | Invalid_argument _ ->
     raise (TypeDefinitionError
              (Printf.sprintf "Wrong arity for type %s!" name))
...
let derecurse_types env venv defs =
  ...
  let rec get_name ~nd args name =
    ...
    | def, params, lst ->
       if nd then
         raise (TypeDefinitionError
                  (Printf.sprintf "Cannot use a reference to %s here!" name)) ;
       let cached = lst
                    |> List.find_opt (fun (args',_) ->
                           try List.for_all2 equiv args args'
       ...
       | None ->
          begin try
              ...
              let local = List.combine params args
                          |> List.to_seq |> StrMap.of_seq in
