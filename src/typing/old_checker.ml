...
(* TODO: Improve error messages
   (when failing due to all branches having failed, print errors of the
    branches) *)
...
let rec typeof_a ~legacy pos tenv env a =
  ...
  | Projection (Field label, v) ->
     ...
     if subtype t record_any then
       ...
       with Not_found ->
         raise (Ill_typed (pos, "Label " ^ label ^ " not present."))
     else
  ...
  | Lambda (va, Ast.ADomain s, v, e) ->
     ...
     if equiv s dom
     then inferred_t
     else raise (Ill_typed
                   (pos,
                    "The inferred domain for the abstraction is different. "
                    ^(actual_expected dom s)))
  | Lambda (va, Ast.AArrow t, v, e) ->
     let inferred_t = type_lambda env va v e in
     if subtype inferred_t t
     then t
     else raise (Ill_typed
                   (pos, "The inferred type for the abstraction is too weak. "
                         ^(actual_expected inferred_t t)))
...
let propagate tenv x a gammas =
  gammas |>
    List.map (fun gamma ->
        ...
        then refine_a ~sufficient:false tenv gamma a any
               (Env_refinement.find x gamma)
        else [gamma]
      ) |> List.flatten
...
let restrict_annots_a gamma =
  map_a
    (function Bind (va, v, a, e) ->
               Bind (VarAnnot.restrict gamma va, v, a, e) | e -> e)
    (function Lambda (va, t, v, e) ->
               Lambda (VarAnnot.restrict gamma va, t, v, e) | a -> a)
...
let rec infer_legacy' tenv env e t =
  ...
  match e with
  | Var v -> (e, [Env_refinement.refine_existing v t envr]
  ...
  | Bind (va, v, a, e) ->
     ...
     let res =
       ...
       then begin (* BindArgSkip *)
           ...
           let dom_a = (* NOTE: Not in the paper. Used to forward type
                        information for explicitely typed lambdas with
                        multiple arguments *)
             if e = Var v
             then cap_o dom_a t
             else dom_a
           ...
           then begin (* BindArgUntyp *)
               log "@,Skipping definition." ;
               let (e, gammas, changes) = infer_legacy' tenv env e t in
               (Bind (VarAnnot.empty, v, a (* Should be empty already *), e),
                gammas, changes (* true *) (* Optimisation *))
             end
           else if List.exists (fun envr -> Env_refinement.is_empty envr |> not)
                     gammas_a
           then begin (* BindArgRefEnv *)
               ...
               log "@,Using the following split: %a"
                 (Utils.pp_list Cduce.pp_typ) splits ;
               let res =
                 splits
                 |> List.map (fun s ->
                        let (e, gammas, changes) =
                          infer_legacy' tenv (Env.add v s env) e t in
                        let changes =
                          if List.length gammas >= 1
                             && List.for_all Env_refinement.is_empty gammas
                          then changes (* The current annotation will not
                                        change *)
                          else true (* The current annotation (or a parent) will
                                     change *)
                        in
...
and infer_legacy_a' (*pos*)_ tenv env a t =
  ...
  let type_lambda va lt v e t ~maxdom =
    ...
    let res =
      ...
      | [arrows] -> (* Abs *)
         (* NOTE: Here we ignore the negative part, though we should check there
            is no negative part.  But it would require a better simplification
            of union of arrow types to make negative parts disappear. *)
         ...
         then (Lambda (VarAnnot.empty, lt, v, empty_annots_e e), [],
               false (* Optimisation *))
         else
           ...
           log "@,Using the following split: %a" (Utils.pp_list Cduce.pp_typ)
             splits ;
           let res =
             splits
             |> List.map (fun si ->
                    let t = List.filter (fun (sj,_) -> subtype si sj) arrows
                            |> List.map snd |> conj_o in
                    let (e, gammas, changes) =
                      infer_legacy' tenv (Env.add v si env) e t in
                    let changes =
                      if List.length gammas >= 1
                         && List.for_all Env_refinement.is_empty gammas
           ...
           else (Lambda (VarAnnot.empty, lt, v, empty_annots_e e), [],
                 false (* Optimisation *))
      | lst -> (* AbsUntypeable *)
         if lst <> []
         then Format.printf "Warning: An AbsUnion rule would be needed..." ;
         (Lambda (VarAnnot.empty, lt, v, empty_annots_e e), [],
          false (* Optimisation *))
    in
         ...
         then (a, [envr], false)
           ...
           let gammas =
             ...
             |> List.filter_map (fun (ti,si) ->
                    envr |>
                      option_chain
                        [Env_refinement.refine_existing v1 ti ;
                         Env_refinement.refine_existing v2 si]
                  )
           in
           (a, gammas, false)
       end else (a, [], false)
         ...
         if is_empty vt || is_empty ft then (a, [envr], false)
           ...
           let gammas =
             ...
             |> List.filter_map (fun ti ->
                    ...
                    let ti = remove_field_info ti label in
                    envr |>
                      option_chain [Env_refinement.refine_existing v ti ;
                                    Env_refinement.refine_existing f si]
                  )
           in
           (a, gammas, false)
       end else (a, [], false)
  | Ite (v, s, v1, v2) ->
     if Env.mem v env then begin
         ...
         if is_empty vt then (a, [envr], false)
           ...
           let gammas =
             [ envr |> option_chain [Env_refinement.refine_existing v s ;
                                     Env_refinement.refine_existing v1 t] ;
               envr |> option_chain [Env_refinement.refine_existing v (neg s) ;
                                     Env_refinement.refine_existing v2 t] ]
             |> filter_options
           in
           (a, gammas, false)
       end else (a, [], false)
     ...
     if Env.mem v1 env && Env.mem v2 env then begin
         ...
         then (a, [envr], false)
           ...
           | [arrows] -> (* AppR *)
              let gammas =
                arrows
                |> List.filter_map (fun (si,_) ->
                       let arrow_type =
                         mk_arrow (cons (cap_o si vt2)) (cons t) in
                       envr |> option_chain
                                 [Env_refinement.refine_existing v1 arrow_type ;
                                  Env_refinement.refine_existing v2 si]
                     ) in
              (a, gammas, false)
           | lst -> (* AppL *)
              let gammas =
                lst |> List.filter_map (fun arrows ->
                           Env_refinement.refine_existing
                             v1 (branch_type arrows) envr
                         ) in
              (a, gammas, false)
       end else (a, [], false)
  | Let (v1, v2) ->
     let gammas =
       [envr |> option_chain
                  [Env_refinement.refine_existing v1 any ;
                   Env_refinement.refine_existing v2 t ]]
       |> filter_options in
     (a, gammas, false)
...
let infer_legacy tenv env e =
  ...
  let e =
    VarSet.fold (fun v acc ->
        Bind (VarAnnot.initial_binding ~legacy:true,
              v, Abstract (var_type [] v env), acc)
      ) fv e in
...
let rec infer_a' pos tenv env a t =
  ...
  let type_lambda va lt v e t ~maxdom =
    ...
    let res =
      ...
      | [arrows] -> (* Abs *)
         (* NOTE: Here we ignore the negative part, though we should check there
            is no negative part.
            But it would require a better simplification of union of arrow types
            to make negative parts disappear. *)
         ...
         let res =
           ...
           |> List.map (fun si ->
                  ...
                  let t = List.filter (fun (sj,_) -> subtype si sj) arrows
                          |> List.map snd |> conj_o in
                  let (e, gammas) =
                    infer_iterated tenv (Env.add v si env) e t in
         ...
         let gammas = List.flatten gammass in
         let changes = List.exists identity changess in
         if VarAnnot.is_empty va |> not
            && subtype (domain t) (VarAnnot.full_domain va)
         then (Lambda (va, lt, v, e), gammas, changes)
         else (* AbsUntypable *)
           (Lambda (VarAnnot.empty, lt, v, empty_annots_e e), [],
            false (* Optimisation *))
  ...
  then begin (* Option *)
      ...
      | Pair (v1, v2) ->
         ...
         then (a, [Env_refinement.refine v1 any envr] |> filter_options, false)
             ...
             let gammas =
               ...
               |> List.filter_map (fun (ti,si) ->
                      envr |>
                        option_chain [Env_refinement.refine v1 ti ;
                                      Env_refinement.refine v2 si] )
             in
             (a, gammas, false)
           end
      | Projection (typ, v) ->
      ...
      | RecordUpdate (v, label, Some f) ->
         ...
         then (a, [Env_refinement.refine v record_any envr]
                  |> filter_options, false)
         else begin
             ...
             let gammas =
               ...
               |> List.filter_map (fun ti ->
                      let si = get_field ti label in
                      let ti = remove_field_info ti label in
                      envr |> option_chain [Env_refinement.refine v ti ;
                                            Env_refinement.refine f si]
                    )
             in
             (a, gammas, false)
           end
      | Ite (v, s, v1, v2) ->
         ...
         let gammas =
           ...
           else [Env_refinement.refine v s envr ;
                 Env_refinement.refine v (neg s) envr]
                |> filter_options
         in
         (a, gammas, false)
      | App (v1, v2) ->
         ...
         then (a, [Env_refinement.refine v1 arrow_any envr]
                  |> filter_options, false)
         else begin
             let vt1 = Env.find v1 env in
             let vt2 = Env.find v2 env in
             match dnf (cap_o vt1 arrow_any) |> simplify_dnf with
             | [arrows] when subtype vt2 (arrows |> List.map fst |> disj_o) ->
                (* AppSplitR *)
                let gammas =
                  arrows
                  |> List.filter_map (fun (si,_) ->
                         let arrow_type =
                           mk_arrow (cons (cap_o si vt2)) (cons t) in
                         envr
                         |> option_chain [Env_refinement.refine v1 arrow_type ;
                                          Env_refinement.refine v2 si]
                       ) in
                (a, gammas, false)
             | [arrows] when (has_absent vt1 || has_absent vt2) |> not ->
                (* AppWrongDom *)
             ...
             | lst -> (* AppSplitL *)
                let gammas =
                  lst
                  |> List.filter_map (fun arrows ->
                         envr
                         |> option_chain
                              [Env_refinement.refine v1 (branch_type arrows) ;
                               Env_refinement.refine v2 any]
                       ) in
                (a, gammas, false)
           end
      | Let (v1, v2) ->
           ...
           then (* BindArgSkip *)
             ...
             (Bind (va, v, restrict_annots_a env a, e), gammas, changes)
               ...
               then begin (* BindArgUntyp *)
                   ...
                   (Bind (VarAnnot.empty, v, a (* Should be empty already *),
                          empty_annots_e e),
                    [], true)
                 end
               else if are_current_env gammas_a |> not
               then begin (* BindArgRefEnv *)
                   log "@,The definition need refinements (going up)." ;
                   let e = restrict_annots_e env e in
                   let va = VarAnnot.restrict env va in
                   ...
                   let to_propagate =
                     ...
                     then
                       splits
                       |> List.map (fun si ->
                              refine_a ~sufficient:false tenv envr a s si)
                       |> List.concat
                     else [envr]
                   in
                   ...
                   then begin (* BindPropagateSplit *)
                       log
                         "@,... but first some constraints must be propagated.";
                       let va =
...
let infer tenv env e =
  ...
  let e = VarSet.fold
            (fun v acc ->
              Bind (VarAnnot.initial_binding ~legacy:false,
                    v, Abstract (var_type [] v env), acc))
            fv e in
  let e =
