...
let get_bind_annots pos v anns =
  match anns with
  | EmptyA -> raise (Ill_typed (pos, "Unrelevant annotation for variable "
                                     ^(Variable.show v)^"."))
  | BindA anns -> anns

let get_lambda_annots pos v anns =
  match anns with
  | UntypAtomA -> raise (Ill_typed (pos, "Untypable annotation for variable "
                                         ^(Variable.show v)^"."))
  | EmptyAtomA | AppA _ ->
     raise (Ill_typed (pos, "Unrelevant annotation for variable "
                            ^(Variable.show v)^"."))
  | LambdaA (_, anns) -> anns
...
let rec typeof_a pos tenv env anns a =
  ...
  let type_lambda env v e =
    ...
    if splits = []
    then raise (Ill_typed (pos, "Empty annotation for variable "
                                ^(Variable.show v)^"."))
    else begin
        LambdaSA.destruct va
        |> List.map (fun (t, (anns, _, _)) ->
  ...
  | Lambda (_, Ast.AArrow t, v, e) ->
     ...
     then t
                   ...
                   (pos,
                    "The inferred type for the abstraction is too weak. "
...
let refine_a ~sufficient tenv env a prev_t t =
  ...
  if not sufficient && is_empty t then []
       ...
       | Const c, false -> if disjoint (typeof_const_atom tenv c) t
                           then [] else [env]
       | Const c, true -> if subtype (typeof_const_atom tenv c) t
                          then [env] else []
       | Pair (v1, v2), _ ->
          split_pair t
          |> List.filter_map (
                 fun (t1, t2) ->
                 env
                 |> option_chain [Env_refinement.refine v1 t1 ;
                                  Env_refinement.refine v2 t2]
               )
       | Projection (Fst, v), _ ->
          [Env_refinement.refine v (mk_times (cons t) any_node) env]
          |> filter_options
       | Projection (Snd, v), _ ->
          [Env_refinement.refine v (mk_times any_node (cons t)) env]
          |> filter_options
       | Projection (Field label, v), _ ->
          [Env_refinement.refine v (mk_record true [label, cons t]) env]
          |> filter_options
       ...
       | RecordUpdate (v, label, Some x), _ ->
          ...
          |> List.filter_map (
                 ...
                 let ti = remove_field_info ti label in
                 env |>
                   option_chain [Env_refinement.refine v ti ;
                                 Env_refinement.refine x field_type]
               )
       | App (v1, v2), _ ->
       ...
       | Ite (v, s, x1, x2), _ ->
          [ env |> option_chain [Env_refinement.refine v s ;
                                 Env_refinement.refine x1 t] ;
            env |> option_chain [Env_refinement.refine v (neg s) ;
                                 Env_refinement.refine x2 t] ]
          |> filter_options
       | Lambda _, false ->
...
let filter_res_a =
  List.filter_map
    (function (None, _) -> None | (Some gamma, anns) -> Some (gamma, anns))
...
let rec infer_a' ?(no_lambda_ua=false) pos tenv env anns a ts =
  ...
  let type_lambda v e ts va ~opt_branches_maxdom ~former_typ =
    ...
    then begin
        ...
        let va = LambdaSA.map_top (fun s t b ->
                     if b
                     then (worst s, t, b)
                     else (substitute_all_jokers s any, t, b)) va
                 |> LambdaSA.normalize
        in
        let splits = va |> LambdaSA.destruct in
        log "@,Using the following splits: %a" pp_lambda_splits
          (LambdaSA.destruct va) ;
        let res =
          splits
          |> List.map (fun (s, (anns, t, b)) ->
                 assert (has_absent s |> not) ;
                 let env = Env.add v s env in
                 let res = infer_iterated tenv env anns e t in
                 let changes = exactly_current_env res = None in
                 let res = List.map (fun (gamma, anns) ->
                               (gamma, (anns, worst t, b))) res in
                 (res, changes)
               ) in
        let (ress, changess) = List.split res in
        let changes = List.exists identity changess in
        let res = List.flatten ress
                  |> regroup v
                  |> List.map (fun (gamma, anns_a) ->
                         (gamma,
                          LambdaA (former_typ, LambdaSA.construct anns_a))
                       ) in
        ...
        let branches =
          ts |> List.map dnf
          |> List.map (List.filter (fun arrows ->
                           subtype (branch_type arrows) t)) (* Optimisation *)
          |> List.map simplify_dnf |> List.flatten |> List.flatten
        in
        log "@,Branches suggested by t: %a" pp_branches branches ;
        let va = LambdaSA.enrich ~opt_branches_maxdom ~former_typ
                   (initial_e e) va branches in
        let res = infer_a_iterated ~no_lambda_ua:true pos tenv env
                    (LambdaA (former_typ,va)) a [arrow_any] in
        let best_t =
          res
          |> List.map (fun (_, anns) ->
                 match anns with
        ...
        let res =
          ...
          else (log "@,Cannot obtain the required type: %s"
                  (actual_expected best_t worst_target_t) ; ([], false))
        in
        log "@]@,END LAMBDA for variable %a" Variable.pp v ;
        res
      end
  ...
  then begin (* Option *)
      ...
      let res =
        if subtype any (disj ts)
           && List.for_all (fun (gamma,_) ->
                  Env_refinement.is_empty gamma |> not) res
        then (envr, UntypAtomA)::res
        else res
      in
      (res, changes)
    end else begin
      ...
      | Projection (typ, v), _ ->
         ...
         in
         let res = [(Env_refinement.refine v t envr, EmptyAtomA)]
                   |> filter_res_a in
         (res, false)
      | RecordUpdate (v, label, None), _ ->
      ...
      | RecordUpdate (v, label, Some f), _ ->
         ...
         let res =
           ...
           then
             [(envr |> option_chain
                         [Env_refinement.refine v record_any ;
                          Env_refinement.refine f any],
               EmptyAtomA)]
             |> filter_res_a
           else begin
      ...
      | App (v1, v2), AppA (t',b) ->
         ...
         let res =
           if is_empty vt1
           then [(Env_refinement.refine v2 any envr, AppA (t',b))]
                |> filter_res_a
           else if is_empty vt2
           then [(Env_refinement.refine v1 arrow_any envr, AppA (t',b))]
                |> filter_res_a
           else begin
               ...
               | [_] when can_refine_l -> (* AppRefineL *)
                  let arrow_type = mk_arrow
                                     (cons (cap vt2 (joker Max)))
                                     (cons (cap t (joker Min))) in
      ...
      | Let (v1, v2), _ ->
         let res =
           ...
           |> filter_res_a in
         (res, false)
      | Lambda (_, ua, v, e), LambdaA (former_typ, va)
           when ua = Ast.Unnanoted || no_lambda_ua ->
      ...
      | Lambda (_, Ast.AArrow s, v, e), LambdaA (former_typ, va)
           when subtype s worst_t ->
...
and infer' tenv env anns e' t =
  ...
  match e', anns with
  | Var v, _ -> ([(Env_refinement.refine v t envr, EmptyA)]
                 |> filter_res, false)
  | Bind (_, v, a, e), BindA (anns_a, va) ->
     ...
     let res =
       ...
       | [(s, anns)] when subtype any_or_absent s -> (* BindDefSkip *)
          ...
          let res = infer_iterated tenv env anns e t in
          let changes = exactly_current_env res = None in
          let res = regroup v res
                    |> List.map (fun (gamma, anns) ->
                           (gamma, BindA (anns_a, BindSA.construct anns))
                         ) in
          (res, changes)
       | splits ->
          ...
          | Some anns_a ->
             ...
             (* if subtype s dom_a |> not
                then Format.printf "%s@." (actual_expected s dom_a) ;
              *)
             ...
             let rec propagate lst treated =
               ...
               | (ns,anns)::lst ->
                  ...
                  then propagate lst ((ns,anns)::treated)
                    ...
                    let sufficient = refine_a ~sufficient:true tenv envr a s
                                       (cap_o (neg ns) s) in
             ...
             | Some ((s,anns), splits, necessary, sufficient) ->
                (* BindPropagate *)
                log "@,Some constraints must be propagated." ;
                let res1 = necessary
                           |> List.map (fun gamma ->
                                  let anns = (s,anns)::splits
                                             |> BindSA.construct in
                ...
                (res1@res2, true)
             | None -> (* Bind *)
                log "@,Using the following splits: %a" pp_splits
                  (List.map fst splits) ;
                let res =
...
and infer_a_iterated ?(no_lambda_ua=false) pos tenv env anns a ts =
  ...
  | (res, true) ->
     ...
     | None -> res
     | Some anns -> infer_a_iterated ~no_lambda_ua pos tenv env anns a
                      (List.map worst ts)
     end
  | (res, _) -> res
...
and infer_iterated tenv env anns e t =
  ...
  | (res, true) ->
     ...
     | None -> res
     | Some anns ->
        infer_iterated tenv env anns e t (* t should not contain jokers *)
     end
  | (res, _) -> res
...
let infer tenv env e =
  ...
  let e = VarSet.fold (fun v acc ->
              Bind (Old_annotations.VarAnnot.empty, v,
                    Abstract (var_type [] v env), acc)
            ) fv e in
  let anns =
